...
namespace Ced\Ebay\Model\ResourceModel;
...
class Profileproducts extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function _construct()
    {
        ...
        $this->_init('ebay_profile_products', 'id');
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('ebay_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('Ebay/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('ebay_profile_products'), $condition);
        return $this;
    ...
    {
        if ( $productId > 0 ) {
            $profileTable = $this->getTable('ebay_profile_products');

            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()->get('Ced\Ebay\Model\Profileproducts')->loadByField('profile_id',$profileId);
