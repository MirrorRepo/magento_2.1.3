...
namespace Ced\Ebay\Ui\Component\Listing\Columns;
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                if (isset($item[$fieldName])) {
                    if ( $item[$fieldName] == 'Valid' || is_null($item[$fieldName])) {
                        ...
                        $errors = "";
                        try{
                            if(empty($item['ebay_validation_errors'])) {
                                $productArray = $this->product->load($item['entity_id'])->toArray();
                                $item['ebay_validation_errors'] = isset($productArray['ebay_validation_errors']) ? $productArray['ebay_validation_errors'] : '';
                            }
                            if (!empty($item['ebay_validation_errors'])) {
                                $errorsArray = $this->json->jsonDecode($item['ebay_validation_errors']);
